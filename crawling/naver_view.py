...
# url = "https://search.naver.com/search.naver?where=view&sm=tab_jum&query={}&start={}".format(keyword, start_page)
# r = requests.get(url)
# bs = BeautifulSoup(r.text, "lxml")

# results = []
# for li in bs.select("li._svp_item"):
#         try:
...
def get_naver_view_search(keyword, start_page=1, end_page=None):
    '''
    네이버 블로그 검색 함수
    ...
    if start_page < end_page:
        ...
        results.extend(get_naver_view_search(keyword, start_page, end_page))    

    return results

# 함수 호출
results = get_naver_view_search("파이썬강좌", 1)
